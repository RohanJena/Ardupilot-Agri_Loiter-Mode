...
uint64_t * check_uint64_t(lua_State *L, int arg) {
    void *data = luaL_checkudata(L, arg, "uint64_t");
...
static int EFI_State_engine_speed_rpm(lua_State *L) {
    ...
    switch(lua_gettop(L)) {
        case 1:
            new_uint32_t(L);
            *check_uint32_t(L, -1) = ud->engine_speed_rpm;
            return 1;
        case 2: {
            const uint32_t raw_data_2 = coerce_to_uint32_t(L, 2);
            const uint32_t data_2 = static_cast<uint32_t>(raw_data_2);
...
static int EFI_State_last_updated_ms(lua_State *L) {
    ...
    switch(lua_gettop(L)) {
        case 1:
            new_uint32_t(L);
            *check_uint32_t(L, -1) = ud->last_updated_ms;
            return 1;
        case 2: {
            const uint32_t raw_data_2 = coerce_to_uint32_t(L, 2);
            const uint32_t data_2 = static_cast<uint32_t>(raw_data_2);
...
static int RangeFinder__RangeFinder_State_last_reading_ms(lua_State *L) {
    ...
    switch(lua_gettop(L)) {
        case 1:
            new_uint32_t(L);
            *check_uint32_t(L, -1) = ud->last_reading_ms;
            return 1;
        case 2: {
            const uint32_t raw_data_2 = coerce_to_uint32_t(L, 2);
            const uint32_t data_2 = static_cast<uint32_t>(raw_data_2);
...
static int uint64_t_index(lua_State *L) {
    const char * name = luaL_checkstring(L, 2);
    if (load_function(L,uint64_t_meta,ARRAY_SIZE(uint64_t_meta),name)) {
        return 1;
    }
    return 0;
}
