...
void GCS_MAVLINK_Copter::custom_handle_message_set_position_target_local_ned(const mavlink_message_t &msg)
{
        // decode packet
        mavlink_set_position_target_local_ned_t packet;
        mavlink_msg_set_position_target_local_ned_decode(&msg, &packet);
        ...
        if (packet.coordinate_frame != MAV_FRAME_LOCAL_NED &&
            ...
            packet.coordinate_frame != MAV_FRAME_BODY_OFFSET_NED) {
            // input is not valid so stop
            copter.mode_agri_loiter.init(true);
            return;
        }
        ...
        // send request
        // if (!pos_ignore && !vel_ignore) {
        //     copter.mode_.set_destination_posvelaccel(pos_vector, vel_vector, accel_vector, !yaw_ignore, yaw_cd, !yaw_rate_ignore, yaw_rate_cds, yaw_relative);
        // } else if (pos_ignore && !vel_ignore) {
        //     copter.mode_guided.set_velaccel(vel_vector, accel_vector, !yaw_ignore, yaw_cd, !yaw_rate_ignore, yaw_rate_cds, yaw_relative);
        // } else if (pos_ignore && vel_ignore && !acc_ignore) {
        //     copter.mode_guided.set_accel(accel_vector, !yaw_ignore, yaw_cd, !yaw_rate_ignore, yaw_rate_cds, yaw_relative);
        ...
        //     copter.mode_guided.init(true);
        // }
        copter.mode_agri_loiter.set_z_velocity(vel_vector);
}
#endif 
...
#if MODE_AGRILOITER_ENABLED == ENABLED
    case MAVLINK_MSG_ID_SET_POSITION_TARGET_LOCAL_NED:
            custom_handle_message_set_position_target_local_ned(msg);
            break;
#endif
    }
}
